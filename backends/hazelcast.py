...
from common.hazelcast_cache.utils import import_class
from common.hazelcast_cache.cache import CacheKey
...
def parse_url(hazelcast_host):
    """
    Hazelcast_host should be have a format like `admin:admin@localhost:9010`
    """
    try:
        username = password = url = ''
        if '@' in hazelcast_host:
            split = hazelcast_host.rsplit('@', 1)
            infor, url = split[0], split[1]
            if ':' in infor:
                username, password = infor.rsplit(':', 1)
        else:
            return hazelcast_host
    except Exception as e:
        logger.error('Hazelcast_host should be have a format like `admin:admin@localhost:9010` {}'
...
class BaseHazelcastCache(BaseCache):
    ...
    def get_serializer_class(self):
        serializer_class = self.options.get(
            'SERIALIZER_CLASS',
            'common.hazelcast_cache.serializers.DefaultSerializer'
        )
    ...
    def get_compressor_class(self):
        compressor_class = self.options.get(
            'COMPRESSOR_CLASS',
            'common.hazelcast_cache.compressors.NoopCompressor'
        )
    ...
    def make_key(self, key, version=None):
        if not isinstance(key, CacheKey):
            versioned_key = super().make_key(key, version)
